...
| B02b | `BACKEND_ONLY` | receipt/job 查询、activity stream 与真实 SSE router | backlog/heartbeat/Last-Event-ID/410 CURSOR_EXPIRED/断连续传已验收 |
...
| B04a | `BACKEND_ONLY` | 分段上传、原始 bytes HTTP route、真实 100 MiB CAS | 续传、错offset/digest、崩溃恢复与身份注入负链已验收 |
| B04b | `BACKEND_ONLY` | CAS Range、日志 byte cursor 与流式 HTTP routes | 200/206/416、分段查看、封口工件与公开日志白名单已验收 |
| B05a | `BACKEND_ONLY` | 单组织四窄身份、双身份切换及 identity routes | cookie/session派生principal与伪造负链已验收 |
