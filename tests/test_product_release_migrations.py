...
def test_project_release_manifest_empty_install_and_repeat_are_exact(
    ...
    assert len(manifest.fragments) == 35
    assert manifest.fragments[-2].fragment_id == "B17b/pool_artifacts"
    assert manifest.fragments[-1].fragment_id == "B05d/managed_sessions"
...
def test_release_upgrades_the_previous_34_fragment_database_atomically(
    tmp_path: Path,
) -> None:
    old_root = tmp_path / "old-fragments"
    shutil.copytree(FRAGMENTS, old_root)
    (old_root / "B05d" / "managed_sessions.sql").unlink()
    (old_root / "B05d").rmdir()
    old_manifest, old_lock = tmp_path / "old-manifest.json", tmp_path / "old.lock"
    current_order = tuple(
        item.fragment_id for item in _release(FRAGMENTS, MANIFEST, LOCK).manifest().fragments[:34]
    )
    ...
    with sqlite3.connect(db, isolation_level=None) as connection:
        ...
        assert connection.execute(
            "SELECT name FROM sqlite_master WHERE name='product_managed_sessions'"
        ).fetchone() == ("product_managed_sessions",)
